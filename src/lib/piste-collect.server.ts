...
export async function runPisteCollection(input: {
  ...
  const { data: job, error: jobError } = await supabase
    ...
    .insert({
      ...
      run_params: { api: "piste-legifrance", query: input.query, maxItems: input.maxItems },
    })
    .select("id")
    .single();
  ...
  try {
    const hits = (await searchLoda({ query: input.query, pageSize: input.maxItems })).slice(
      0,
      input.maxItems,
    );
    ...
    for (const hit of hits) {
      try {
        ...
        const { error } = await supabase.from("raw_items").insert({
          ...
          source_id: source.id,
          source_url: url,
          raw_payload: { hit, text, plain_text: content } as unknown as never,
    ...
    return { jobId: job.id, apifyRunId: null, ingested, duplicates, failed };
  } catch (error) {
